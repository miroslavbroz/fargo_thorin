...
PlanetarySystem *sys;
{
  ...
  real Xplanet, Yplanet, Zplanet, Mplanet, VXplanet, VYplanet, VZplanet;
  real facc, facc1, facc2, frac1, frac2; /* We adopt the same notations as W. Kley */
  real *dens, *abs, *ord, *vrad, *vtheta;
  real PxPlanet, PyPlanet, PzPlanet, vrcell, vtcell, vxcell, vycell, xc, yc;
  real dPxPlanet, dPyPlanet, dPzPlanet, dMplanet;
  nr     = Rho->Nrad;
  ns     = Rho->Nsec;
  dens   = Rho->Field;
  ...
  for (k=0; k < sys->nb; k++) {
    if (sys->acc[k] > 1e-10) {
      dMplanet = dPxPlanet = dPyPlanet = dPzPlanet = 0.0;
