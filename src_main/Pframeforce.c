...
i,j,k,l,m) \
  ...
  real axpl_priv[sys->nb], aypl_priv[sys->nb];
  for (k=0; k<npl; k++) {
    axpl_priv[k] = 0.0;
    aypl_priv[k] = 0.0;
  }
...
//    }
      ...
      if (DampFlux == YES) {
        if (i==0)
          rhoinf = SigmaMed[i]-0.5*(SigmaMed[i+1]-SigmaMed[i]);
        else
          rhoinf = 0.5*(SigmaMed[i-1]+SigmaMed[i]);
        vr[l] -= GASFLUX/(2.0*PI*ri*rhoinf);
      } else if (ViscosityAlpha) {
        vr[l] -= 3.0*viscosity/r*(-SIGMASLOPE+2.0*FLARINGINDEX+1.0);
      } else {
        vr[l] -= 3.0*viscosity/r*(-SIGMASLOPE+.5);
      }
