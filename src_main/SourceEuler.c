...
#pragma omp for
  ...
  if ((Damping == NO) || ((Damping == YES) && (DampVrad == YES))) ImposeKeplerianEdges (VthetaInt);
}
...
real deltaT;
{
  ...
  for (i = One_or_active; i < Max_or_active; i++) {
	...
	invdt5 = FViscosity(Rmed[i])*4./pow(min2(dxrad,dxtheta),2);
      ...
      if (!Pebbles) {
	dt = CFLSECURITY/sqrt(invdt1*invdt1+invdt2*invdt2+invdt3*invdt3+invdt4*invdt4+invdt5*invdt5+invdtreb_sq);
      } else {
